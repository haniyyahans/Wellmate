...
class LaporanController extends Controller
{
    ...
    // private $targetMingguan = 15500;  // ditentukan manual 
    // private $targetBulanan = 57500;   // ditentukan manual 

    public function __construct() {
        if (!isset($_SESSION['id_pengguna'])) { // Cek session, kalau belum ada redirect ke login atau set default
            header('Location: index.php?c=Berita&m=index');
            exit;
        }
        $this->laporanModel = $this->model('Laporan'); // Ambil target dari database sesuai user yang login
        $id_pengguna = $_SESSION['id_pengguna'];
        $targetHarian = $this->laporanModel->getTargetUser($id_pengguna);
    ...
    { // menampilkan halaman utama laporan
        ...
        $this->view('HalamanLaporan.php', $data);
    }

    private function perbaruiLaporanOtomatis($id_pengguna)
    { // agar laporannya selalu update
    ...
    { // memaksa membuat ulang laporan mingguan & bulanan, untuk testing.
        $id_pengguna = $_SESSION['id_pengguna'];                   // ambil data dari sesion
        $this->buatLaporanMingguan($id_pengguna);                  // buat laporan mingguan
        $this->buatLaporanBulanan($id_pengguna);                   // buat laporan bulanan
        header('Location: index.php?c=LaporanController&m=index'); // redirect ke halaman laporan
        exit;
    }
}
