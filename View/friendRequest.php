                                    ...
                                    <div class="request-item flex items-center justify-between p-4 bg-gray-50 rounded-lg shadow-sm">
                                        ...
                                        <div class="flex space-x-2">
                                            <form action="index.php?c=Friend&m=acceptRequest" method="POST">
                                                <input type="hidden" name="id_teman" value="<?= $request['id_teman'] ?>">
                                                <button type="submit" 
                                                        class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition duration-150 text-sm">
                                            ...
                                            <form action="index.php?c=Friend&m=declineRequest" method="POST">
                                                <input type="hidden" name="id_teman" value="<?= $request['id_teman'] ?>">
                                                <button type="submit" 
                                                        class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 transition duration-150 text-sm">
    ...
    <script>
        ...
        document.getElementById('confirmDelete').addEventListener('click', function() {
            if (requestToDelete) {
                ...
                requestToDelete.style.transition = 'all 0.5s ease';
                requestToDelete.style.transform = 'translateX(-100%)';
                requestToDelete.style.opacity = '0';
                
                setTimeout(() => {
                    requestToDelete.remove();
                    requestToDelete = null;
                    
                    // Update badge and check empty state
                    updateBadgeCount();
                    checkEmptyState();
                }, 500);
                ...
                // Show notification
                showNotification(`Permintaan pertemanan dari ${friendName} ditolak`, 'error');
            }
        });
        ...
        document.getElementById('confirmAccept').addEventListener('click', function() {
            if (requestToConfirm) {
                ...
                requestToConfirm.style.transition = 'all 0.5s ease';
                requestToConfirm.style.transform = 'scale(1.05)';
                requestToConfirm.style.backgroundColor = '#dcfce7';
                
                setTimeout(() => {
                    requestToConfirm.style.transform = 'translateX(100%)';
                    requestToConfirm.style.opacity = '0';
                    
                    setTimeout(() => {
                        requestToConfirm.remove();
                        requestToConfirm = null;
                ...
                // Show notification
                showNotification(`${friendName} sekarang adalah teman Anda!`, 'success');
            }
        });
